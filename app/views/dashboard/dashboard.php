...
/** Serie morosidad últimos 6 meses */
$stmt = $conexion->query("
...
$stmt = $conexion->query("
  SELECT COUNT(*) 
  FROM reserva 
  WHERE estado='APROBADA' AND fecha_inicio::date <= CURRENT_DATE 
");
$reservas_activas = (int)$stmt->fetchColumn();

/** (Opcional) Aprobadas del mes, por si quieres mostrarlo como subtítulo */
$stmt = $conexion->query("
  SELECT COUNT(*)
  FROM reserva 
...
/** ========== ACTAS (KPI NUEVO) ========== */
$actas_por_entregar = 0;
$actas_por_devolver = 0;
try {
  // Por entregar: reservas aprobadas SIN acta
  $stmt = $conexion->query("
    SELECT COUNT(*)
    FROM reserva r
    LEFT JOIN acta_reserva a ON a.id_reserva = r.id_reserva
    WHERE r.estado='APROBADA' AND a.id_acta IS NULL
  ");
  $actas_por_entregar = (int)$stmt->fetchColumn();

  // Por devolver: actas ENTREGADO (se entregó el área, falta recibir)
  $stmt = $conexion->query("
    SELECT COUNT(*) FROM acta_reserva WHERE estado='ENTREGADO'
  ");
  $actas_por_devolver = (int)$stmt->fetchColumn();
} catch (PDOException $e) {
  // Si aún no existe acta_reserva, mostramos 0 sin romper el panel
  $actas_por_entregar = 0;
  $actas_por_devolver = 0;
}
...
<head>
  ...
  <style>
    ...
    .user-chip-sm{
      background:#e9f7f6; color:#0F3557; border-radius:18px; padding:.25rem .45rem; font-weight:600;
...
<div class="container-xxl my-4">
  ...
  <div class="row g-3">
    ...
    <!-- Reservas (UN SOLO CARD) -->
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card card-kpi p-3">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <div class="kpi-sub">Reservas activas (ahora)</div>
            <div class="kpi-value"><?= number_format($reservas_activas) ?></div>
            <div class="text-muted small">Aprobadas este mes: <?= number_format($reservas_mes) ?></div>
          </div>
    ...
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card card-kpi p-3">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <div class="kpi-sub">Actas (operación)</div>
            <div class="kpi-value"><?= number_format($actas_por_entregar + $actas_por_devolver) ?></div>
            <div class="text-muted small">
              Por entregar: <?= number_format($actas_por_entregar) ?> · Por devolver: <?= number_format($actas_por_devolver) ?>
            </div>
          </div>
          <div class="icon"><i class="bi bi-journal-check"></i></div>
        </div>
        <div class="mt-3">
          <a href="../operacion/actas.php" class="btn btn-sm btn-domus">Ir a Actas</a>
        </div>
      </div>
    </div>
    ...
    <div class="col-12">
      <div class="card card-chart p-3">
        <div class="d-flex flex-wrap gap-2">
          ...
          <a href="usuarios.php" class="btn btn-outline-primary"><i class="bi bi-people me-1"></i> Usuarios</a>
          <a href="reservas.php" class="btn btn-outline-primary"><i class="bi bi-calendar-check me-1"></i> Reservas</a>
          <a href="../operacion/actas.php" class="btn btn-outline-primary"><i class="bi bi-clipboard-check me-1"></i> Actas</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  new Chart(ctxP, {
    type: 'bar',
    data: { labels: labelsPagos, datasets: [{ label: 'Bs', data: dataPagos }] },
